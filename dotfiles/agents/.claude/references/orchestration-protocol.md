...
2. **Reports Path**: the hook-injected `Reports:` path for that project
3. **Plans Path**: the hook-injected `Plans:` path

Never construct these by hand — the layout differs per repo (`.workbench/` feature-first vs legacy `plans/`) and only the hook knows which applies.

**Example:**
```
Task prompt: "Fix parser bug.
Work context: /path/to/project-b
Reports: <injected Reports: path>
Plans: <injected Plans: path>"
```
