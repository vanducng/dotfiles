...
- **Engineering journals** (personal dev log, retros, post-mortems) → live in the hook-injected `Journals:` path. `./docs/` is for artifacts shared with the team; journals are first-person and ephemeral.
- **Plans, phase files, scout/research/review reports** → live under the hook-injected `Plans:`/`Reports:` paths.
...
### Plan Location
Save plans in the hook-injected `Plans:` path, with timestamp and descriptive name. Never construct the path by hand.

**Format:** use the naming pattern from the injected `## Naming` section.

**Example:** `<injected Plans: path>/251101-1505-authentication-and-profile-implementation/`

**Feature-first layout (opt-in):** when a repo sets `paths.layout: "feature-first"` in `.vd.json`, the injected `Plans:`/`Reports:`/`Visuals:`/`Journals:`/`State:` paths nest under one per-feature folder — `.workbench/features/{ticket}-{slug}/{plans,reports,visuals,journals,state}/` — instead of flat type-first siblings, with root-level `_global/` (cross-feature) and `_archive/` (done) zones beside `features/`. Always write to the injected path; never construct the layout yourself. Create/list/archive feature folders with the `workbench` skill.
