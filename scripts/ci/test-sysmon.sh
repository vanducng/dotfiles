...
grep -q 'KITTY_GROUP:-sysmon' "$SCRIPT" || fail "sysmon should isolate its kitty group"
grep -q 'KITTY_GROUP=herdr-bar' "$SCRIPT" || fail "sidebar should use its own kitty group"
...
printf '%s\n' "$out" | grep -q 'open: herdr-bar' || fail "dia should open the herdr window: $out"
printf '%s\n' "$out" | grep -q 'attach -t sysmon' && fail "dia sidebar must not attach tmux: $out"
out="$("$SCRIPT" dia-right)"
printf '%s\n' "$out" | grep -q 'side: right' || fail "dia-right side: $out"
grep -q 'split_hub_open' "$SCRIPT" || fail "a closed hub must rebuild the split instead of restoring"
grep -q 'focus_bar_if_placed' "$SCRIPT" || fail "a split that is already in place must focus the sidebar"
awk '/^cmd_show\(\)/,/^}/ { print }' "$SCRIPT" | grep -q 'cmd_dia_off' && fail "meh+f must not turn off the herdr split"
grep -q 'close_bar floating' "$SCRIPT" || fail "meh+f should only close a floating herdr window"
