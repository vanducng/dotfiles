...
[[ "$(cat "$capture")" == $'w1:p1\twidget:fix-auth-token-refresh' ]]

run env HERDR_RENAME_CWD="$git_dir/subdir" \
  HERDR_RENAME_CLAUDE_BIN="$test_dir/claude-long" HERDR_RENAME_CODEX_BIN="$missing"
[[ "$(cat "$capture")" == $'w1:p1\twidget:investigate-flaky' ]]
...
run env HERDR_RENAME_CWD="$long_dir" \
  HERDR_RENAME_CLAUDE_BIN="$test_dir/claude-long" HERDR_RENAME_CODEX_BIN="$missing"
[[ "$(cat "$capture")" == $'w1:p1\tcnb-web-services-suite:investigate-flaky' ]]
...
[[ "$(cat "$capture")" == $'w1:p1\twidget:ship-codex-rename' ]]
