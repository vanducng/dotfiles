...
grep -q 'bind-key -n C-M-h select-pane -L' "$CONF" || fail "tmux should bind Ctrl-Alt-h to the left pane"
grep -q 'bind-key -n C-M-j select-pane -D' "$CONF" || fail "tmux should bind Ctrl-Alt-j to the pane below"
grep -q 'bind-key -n C-M-k select-pane -U' "$CONF" || fail "tmux should bind Ctrl-Alt-k to the pane above"
grep -q 'bind-key -n C-M-l select-pane -R' "$CONF" || fail "tmux should bind Ctrl-Alt-l to the right pane"
