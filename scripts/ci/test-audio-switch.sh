...
grep -q 'acquire_dialog_lock' "$SCRIPT" || fail "picker is not a singleton"
pass "picker reuses an open dialog"
...
mkdir -p "$workdir/busy.lock"
printf '%s\n' "$$" >"$workdir/busy.lock/pid"
before="$(cat "$AUDIO_SWITCH_STATE/output")"
out="$(AUDIO_SWITCH_LOCK="$workdir/busy.lock" "$SCRIPT" pick)" || fail "busy pick exited: $out"
[[ -z "$out" ]] || fail "busy pick switched: $out"
[[ "$(cat "$AUDIO_SWITCH_STATE/output")" == "$before" ]] || fail "busy pick changed output"
pass "pick ignores a second open"
